...
class TestDetailsAPI {
  ...
  Future<List<TestDetail>> findByTestId(int id) async {

    var respone = await http.get(Uri.parse(BaseUrl.url + "testDetails/findByTestId/" + id.toString()));
    if (respone.statusCode == 200) {
      List<dynamic> res = jsonDecode(utf8.decode(respone.bodyBytes));
      return res.map((e) => TestDetail.fromMap(e)).toList();
    } else {
